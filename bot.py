...
class CommanderBot(commands.Bot):
    ...
    def update_overlay(self):
        try:
            notes = self.brain.get_scout_notes(
                self.current_opponent) if "Madden" in self.current_game and self.current_opponent else []
            data = {
    ...
    def trigger_manual_clip(self):
        ...
        asyncio.run_coroutine_threadsafe(
            self.create_twitch_highlight("Manual clip triggered by Krzy Budz from the BMG Command Center!"), self.loop)
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                        "https://na01.safelinks.protection.outlook.com/?url=https%3A%2F%2Fid.twitch.tv%2Foauth2%2Fvalidate&data=05%7C02%7C%7Cd4a83c422b874a81c3ec08de7bb55c40%7C84df9e7fe9f640afb435aaaaaaaaaaaa%7C1%7C0%7C639084216019240226%7CUnknown%7CTWFpbGZsb3d8eyJFbXB0eU1hcGkiOnRydWUsIlYiOiIwLjAuMDAwMCIsIlAiOiJXaW4zMiIsIkFOIjoiTWFpbCIsIldUIjoyfQ%3D%3D%7C0%7C%7C%7C&sdata=84VOEB4LjKBU0p77vDF7e3DKz9%2Fy3%2Fe7LprnUkfQsKI%3D&reserved=0",
                        headers={"Authorization": f"OAuth {token}"}) as resp:
                ...
                async with session.post(
                        "https://na01.safelinks.protection.outlook.com/?url=https%3A%2F%2Fapi.twitch.tv%2Fhelix%2Fstreams%2Fmarkers&data=05%7C02%7C%7Cd4a83c422b874a81c3ec08de7bb55c40%7C84df9e7fe9f640afb435aaaaaaaaaaaa%7C1%7C0%7C639084216019298737%7CUnknown%7CTWFpbGZsb3d8eyJFbXB0eU1hcGkiOnRydWUsIlYiOiIwLjAuMDAwMCIsIlAiOiJXaW4zMiIsIkFOIjoiTWFpbCIsIldUIjoyfQ%3D%3D%7C0%7C%7C%7C&sdata=DZEC56vIu%2BmktIi8QYr%2B892r6Hnb%2BjcFeeKPwrnyWqI%3D&reserved=0",
                        headers=helix_headers, json=marker_payload) as resp:
                    if resp.status == 200:
                ...
                async with session.post(
                        "https://na01.safelinks.protection.outlook.com/?url=https%3A%2F%2Fapi.twitch.tv%2Fhelix%2Fclips&data=05%7C02%7C%7Cd4a83c422b874a81c3ec08de7bb55c40%7C84df9e7fe9f640afb435aaaaaaaaaaaa%7C1%7C0%7C639084216019353050%7CUnknown%7CTWFpbGZsb3d8eyJFbXB0eU1hcGkiOnRydWUsIlYiOiIwLjAuMDAwMCIsIlAiOiJXaW4zMiIsIkFOIjoiTWFpbCIsIldUIjoyfQ%3D%3D%7C0%7C%7C%7C&sdata=%2F8OqaZsVcqiYSgqxX66COxTfzR6TjivEGaKrIMFNewA%3D&reserved=0",
                        headers=helix_headers, json=clip_payload) as resp:
                    if resp.status == 202:
                        ...
                        if channel:
                            await channel.send(
                                f"🎬 BMG COMMAND CENTER CAUGHT THAT ON TAPE! https://na01.safelinks.protection.outlook.com/?url=https%3A%2F%2Fclips.twitch.tv%2F&data=05%7C02%7C%7Cd4a83c422b874a81c3ec08de7bb55c40%7C84df9e7fe9f640afb435aaaaaaaaaaaa%7C1%7C0%7C639084216019407355%7CUnknown%7CTWFpbGZsb3d8eyJFbXB0eU1hcGkiOnRydWUsIlYiOiIwLjAuMDAwMCIsIlAiOiJXaW4zMiIsIkFOIjoiTWFpbCIsIldUIjoyfQ%3D%3D%7C0%7C%7C%7C&sdata=VIVJSttmeNh2tqSC2qBlriJ7SHbkLSmyaakhlhbcEdQ%3D&reserved=0{clip_id}")
                    else:
    ...
    async def edge_tripwire_loop(self):
        ...
        while self.running:
            ...
            tripwire_words = ["touchdown", "intercept", "fumble", "sacked", "it's good", "final score", "flag",
                              "turnover"]
            triggered = any(word in audio_text for word in tripwire_words) or any(
                word in ocr_text.lower() for word in tripwire_words)
            ...
            if triggered:
                ...
                if self.current_bet_display:
                    result = await asyncio.to_thread(self.strategist.check_bet_resolution, "temp_tripwire.jpg",
                                                     self.current_game, self.current_bet_display, ocr_text)
                    if result and result.get("status") == "resolved" and result.get("winning_key"):
                        ...
                        if channel:
                            if winners > 0:
                                await channel.send(
                                    f"🏁 AI REFEREE: Option {win_key.upper()} wins! Paid out ${payout} to {winners} viewers.")
                            else:
                                await channel.send(f"🏁 AI REFEREE: Option {win_key.upper()} wins! The house sweeps.")
    ...
    def _run_tripwire_analysis(self, ocr_text, audio_text):
        ...
        notes = self.brain.get_scout_notes(
            self.current_opponent) if "Madden" in self.current_game and self.current_opponent else None
    ...
    async def watcher_payout_loop(self):
        ...
        while self.running:
            ...
            if paid_count > 0 and channel:
                await channel.send(
                    f"💸 VIRTUAL VEGAS PAYOUT: Dropped ${payout_amount} into the accounts of {paid_count} active viewers! Keep watching and chatting to earn more.")
    ...
    async def event_message(self, message):
        ...
        if author == "pokemoncommunitygame":
            ...
            elif f"@{self.nick.lower()}" in msg_text and (
                    "don't have" in msg_text or "0x" in msg_text or "don't own" in msg_text):
                missing_ball = "pokeball"
                if "ultraball" in msg_text:
                    missing_ball = "ultraball"
                elif "greatball" in msg_text:
                    missing_ball = "greatball"
        ...
        if reward_id == VIP_REWARD_ID:
            ...
            if question_text:
                ...
                threading.Thread(target=self.trigger_analysis, kwargs={'user_question': formatted_question},
                                 daemon=True).start()
    ...
    async def auto_sportsbook_loop(self):
        ...
        while self.running:
            ...
            bet_data = await asyncio.to_thread(self.strategist.generate_auto_prop, "temp_auto_bet.jpg",
                                               self.current_game, self.streamer_name, self.gamer_tag, audio_transcript)
            ...
            if bet_data:
                ...
                for key, data in bet_data[
                    "options"].items(): announcement += f"| !bet {key} [amount] for {data['text']} ({data['odds']}x) "
    ...
    def trigger_analysis(self, user_question=None):
        ...
        if user_question:
            self.log_callback("AI", f"🧠 Answering VIP Question...")
        else:
            self.log_callback("AI", f"🧠 The Booth is analyzing the field...")
    ...
    async def handle_bounty(self, ctx: commands.Context):
        ...
        if balance < cost:
            await ctx.send(
                f"💸 @{user}, you're broke! This bounty costs ${cost} but you only have ${balance}. Keep watching to earn more!")
            return
    ...
    async def claim_bonus(self, ctx: commands.Context):
        ...
        if user in self.claimed_users:
            await ctx.send(
                f"🛑 @{user}, you already claimed your bonus this stream! Keep watching to earn passive payouts every 15 minutes.")
            return
        self.brain.add_funds(user, 500)
        self.claimed_users.add(user)
        await ctx.send(
            f"🎁 @{user} claimed their $500 starting bonus! Type '!bankroll' to check your balance, and '!bet' to play the sportsbook.")
    ...
    def _run_manual_oddsmaker_thread(self):
        audio_transcript = self.ears.get_transcript()
        bet_data = self.strategist.generate_prop_bet("temp_bet.jpg", self.current_game, self.streamer_name,
                                                     self.gamer_tag, audio_transcript)
        if bet_data:
            self.current_bet_display = bet_data
            market_dict = {key: data["odds"] for key, data in bet_data["options"].items()}
            self.brain.open_market(market_dict, bet_data['question'])
            ...
            for key, data in bet_data[
                "options"].items(): announcement += f"| Type '!bet {key} [amount]' for {data['text']} ({data['odds']}x) "
    ...
    async def start_game_market(self, ctx: commands.Context):
        ...
        bet_data = self.strategist.generate_game_props("temp_bet.jpg", self.current_game, self.streamer_name,
                                                       self.gamer_tag)
        if bet_data:
            self.current_bet_display = bet_data
            market_dict = {key: data["odds"] for key, data in bet_data["options"].items()}
    ...
    async def place_bet(self, ctx: commands.Context):
        ...
        if success:
            await ctx.send(
                f"🎟️ @{user} locked in ${amount} on option {prediction.upper()} (Pays {result_code}x). Balance: ${new_balance}")
        elif result_code == "locked":
            await ctx.send(f"🛑 @{user}, too late! The market is locked.")
        elif result_code == "invalid":
            await ctx.send(f"❌ @{user}, that is not a valid option.")
        elif result_code == "funds":
            await ctx.send(f"❌ @{user}, insufficient funds! You have ${new_balance}.")
    ...
    async def resolve_bet(self, ctx: commands.Context):
        ...
        if winners_count > 0:
            await ctx.send(
                f"🏁 Option {winning_option.upper()} wins! Paid out ${total_paid} to {winners_count} winners.")
        else:
            await ctx.send(f"🏁 Option {winning_option.upper()} wins! The house sweeps the board.")
