...
class CommanderGUI(ctk.CTk):
    ...
    def _init_layout(self):
        ...
        self.lbl_audio_indicator = ctk.CTkLabel(self.frame_audio_dash, text="●", font=("Arial", 24),
                                                text_color="#FF0000")
        ...
        ctk.CTkButton(self.frame_right, text="🎬 CLIP THAT!", fg_color="#1E90FF", height=50, font=("Arial", 16, "bold"),
                      command=lambda: self.bot and self.bot.trigger_manual_clip()).pack(pady=15, padx=10, fill="x")
        ctk.CTkButton(self.frame_right, text="🧠 LAUNCH TACTICAL BOOTH", fg_color="#FF8C00", height=40,
                      font=("Arial", 14, "bold"), command=self.open_coach_popout).pack(pady=5, padx=10, fill="x")
        ctk.CTkButton(self.frame_right, text="📡 VIEW AI INPUTS", fg_color="#8A2BE2", height=40,
                      font=("Arial", 14, "bold"), command=self.open_ai_input_popout).pack(pady=5, padx=10, fill="x")
    ...
    def run_bot_logic(self):
        ...
        try:
            ...
            log_to_gui("CRASH", f"Bot Thread Died: {e}")
        finally:
            loop.close()
    ...
    def open_coach_popout(self):
        ...
        self.txt_coach_notes = ctk.CTkTextbox(self.coach_window, wrap="word", font=("Arial", 16, "bold"),
                                              fg_color="black", text_color="white", border_color="yellow",
                                              border_width=2)
    ...
    def update_coach_popout(self):
        if self.coach_window and self.coach_window.winfo_exists():
            ...
            if self.txt_coach_notes.get("1.0", "end-1c").strip() != text.strip():
                self.txt_coach_notes.delete("1.0", "end");
                self.txt_coach_notes.insert("end", text)
    ...
    def update_video(self):
        if self.bot and getattr(self.bot, 'eyes', None) and self.bot.eyes.get_frame() is not None:
            pil_img = Image.fromarray(
                cv2.cvtColor(cv2.resize(self.bot.eyes.get_frame(), (480, 270)), cv2.COLOR_BGR2RGB))
            if self.current_ctk_image is None:
                ...
                self.current_ctk_image.configure(light_image=pil_img, dark_image=pil_img)
