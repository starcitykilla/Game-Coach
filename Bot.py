...
class CommanderBot(commands.Bot):
    def __init__(self, log_callback):
        ...
        # Ensure VAR Evidence folder exists
    ...
    def update_overlay(self):
        try:
            notes = self.brain.get_scout_notes(
                self.current_opponent) if self.current_game == "Madden" and self.current_opponent else []
            data = {
    ...
    async def create_twitch_highlight(self, reason):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get("https://id.twitch.tv/oauth2/validate",
                ...
                async with session.post("https://api.twitch.tv/helix/streams/markers", headers=helix_headers,
                                        json=marker_payload) as resp:
                    if resp.status == 200:
                ...
                async with session.post("https://api.twitch.tv/helix/clips", headers=helix_headers,
                                        json=clip_payload) as resp:
                    if resp.status == 202:
                        ...
                        if channel:
                            await channel.send(f"🎬 The Booth caught that on tape! https://clips.twitch.tv/{clip_id}")
                    else:
    ...
    def _run_tripwire_analysis(self, ocr_text, audio_text):
        ...
        notes = self.brain.get_scout_notes(
            self.current_opponent) if self.current_game == "Madden" and self.current_opponent else None

        data = self.strategist.analyze("temp_tripwire.jpg", self.current_game, self.streamer_name, self.gamer_tag, None,
                                       notes, ocr_text, chat_string, audio_text)
    ...
    def trigger_analysis(self, user_question=None):
        ...
        data = self.strategist.analyze("temp_frame.jpg", self.current_game, self.streamer_name, self.gamer_tag,
                                       user_question, notes, ocr_text, chat_string, audio_transcript)
